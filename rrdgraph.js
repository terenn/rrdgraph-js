...
(function () {
  ...
  get.config = function () { // {resourceId, report}
...
(function () {
  var Graph = RRDGraph.Graph = function (element, config) {
    ...
    // Canvas Background, TODO: color from config
    var canvas_bg = this.canvas_bg = canvas.append('svg:rect').
      attr('width', this.config.options.width).
      attr('height', this.config.options.height).
      attr('fill', '#fff');
...
(function () {
  ...
  RRDGraph.init = function (selector) {
    d3.selectAll(selector).each(function () {
      var options = parseSrc(this.dataset.src);
      var config_string = RRDGraph.get.config(options);
      var config = RRDGraph.parser.parse(config_string);
    
      var graph = new RRDGraph.Graph(this, config);
